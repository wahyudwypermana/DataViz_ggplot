...
gapminder_plot <- ggplot(gapminder, aes(x = gdpPercap, y = lifeExp, size = pop, color = continent)) +
  geom_point() +
  scale_x_log10() +
  labs(x = "GDP per Capita", y = "Life Expectancy", title = "Life Expectancy vs. GDP per Capita (1952-2007)")

# Create an animated plot
animated_plot <- gapminder_plot +
  facet_wrap(~continent) +
  transition_time(year) +
  labs(title = "Year: {frame_time}")

# Animate the plot
animate(animated_plot, duration = 10, fps = 10, width = 800, height = 600, renderer = gifski_renderer("animated_gapminder.gif"))
```
